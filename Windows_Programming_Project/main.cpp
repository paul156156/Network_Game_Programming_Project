...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdParam, int nCmdShow)
{
    ...
    InitSocket();
    if (RecvInitPos() == false)
    {
        cout << "error RecvInitPos" << endl;
    }
    gameManager->GetPlayer()->SetX(initPos[0]);
    gameManager->GetPlayer()->SetY(initPos[1]);
...
bool RecvInitPos()
{
    ...
    retval = recv(sock, (char*)initPos, len, MSG_WAITALL);
    if (retval == SOCKET_ERROR)
    {
        err_display("recv()");
        return false;
    ...
    cout << "recv init pos" << initPos[0] << initPos[1] << endl;
    return true;
}
void PlayerMove(Fighter& player, SOCKET& sock)
{
