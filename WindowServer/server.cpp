...
int main(int argc, char* argv[])
{
	...
	while (true) {
		...
		if (SendInitPos(info->id) == false)
		{
			cout << "SendInitPos Error" << endl;
		}
...
bool SendInitPos(int _id)
{
	int retval, len, pSize;
	pSize = sizeof(initPos[_id]);
	retval = send(client_sock, (char*)&pSize, sizeof(int), 0);

	cout << "send id: " << _id <<" " << initPos[_id][0] << ',' << initPos[_id][1] << endl;
	...
	retval = send(client_sock, (const char*)initPos[_id], pSize, 0);
	if (retval == SOCKET_ERROR)
	{
		err_display("send()");
		return false;
